...
def update_graph6(year_range):
    ...
    fig.update_layout(
        ...
        xaxis_title="Release Year",
        yaxis_title="Number of Movie")
    return fig


# Figure 7: Rating of TV Show
@app.callback(
...
def update_graph7(year_range):
    ...
    data.columns = ['Rating', 'Count']
    fig = px.bar(data, x='Rating', y='Count', title="Rating of TV Show", color='Rating', 
                 color_discrete_sequence=['#A3C4BC', '#E9AFA3', '#C9CBA3', '#E6B8A2', '#B5C0D0', '#D6A3DC', '#A2D2FF'])
...
def update_graph8(year_range):
    ...
    fig = px.line(t, x='Year', y='Count', color='rating',
                  ...
                  color_discrete_sequence=['#E57373', '#4A90E2', '#81C784', '#F4C542', '#A46BB7', '#F28C28'])
    fig.update_layout(
        title_font=dict(
            family='Arial',
            size=24,
...
def update_graph10(year_range):
    ...
    fig = px.line(m, x='Year', y='Count', color='categories',
                  title="Trends In The Number Of Categories",
                  color_discrete_sequence=['#66A6A0', '#FFAB91', '#9FA8DA', '#FFCC80', '#CE93D8'])
    fig.update_layout(
        title_font=dict(
            family='Arial',
            size=24,
...
def update_graph11(year_range):
    ...
    fig = px.line(v, x='Year', y='Count', color='categories',
                  title="Trends In The Number Of Categories",
                  color_discrete_sequence=['#AFCDE7', '#FFC899', '#A8D5A0', '#C6A5D8', '#F2B5D4'])
